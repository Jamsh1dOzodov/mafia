...
formEl.addEventListener('submit', (event) => {
    ...
    if (emailInp.value.match(emailReg)) {
...
formEl.addEventListener('submit', event => {
    event.preventDefault()
    const nameInp = formEl.name
    const nameReg = /^[a-zA-Z]+$/
    if (nameInp.value.match(nameReg)) {
        nameInp.style.border = '1px solid green'
    }
    else {
        nameInp.style.border = '1px dashed red'
    }
})

formEl.addEventListener('submit', event => {
    event.preventDefault()
    const commentInp = formEl.comment
    const commentReg = /^[A-Za-zА-Яа-яЁё\s]{6,}/

    if(commentInp.value.match(commentReg)){
        commentInp.style.border = '1px solid green'
    }
    else{
        commentInp.style.border = '1px dashed red'
    }
})